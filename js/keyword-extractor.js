...
const supported_language_codes = [
  "ar", "cs", "da", "de", "en", "es", "fa", "fr", "gl",
  "it", "ko", "nl", "pl", "pt", "ro", "ru", "sv", "tr", "vi"
];

// Get stopwords from global variable (like stopwords-en.js)
function getStopwords(options = {}) {
  let _language = options.language || "en";
  _language = sanitize_language(_language);

  if (_language === "en" && window.stopwords_en) {
    return window.stopwords_en;
  }

  console.warn("Stopwords not found for:", _language);
  return [];
}
...
function sanitize_language(requested_language) {
  ...
  if (typeof requested_language !== "string") throw new Error(error_message);

  const _requested_language = (requested_language.length === 2)
    ? requested_language
    : get_language_mapping(requested_language);

  if (!supported_language_codes.includes(_requested_language)) {
...
function get_language_mapping(requested_language) {
  const mapping = {
    ...
    english: "en", french: "fr", galician: "gl", german: "de",
    italian: "it", korean: "ko", persian: "fa", polish: "pl",
    portuguese: "pt", romanian: "ro", russian: "ru", spanish: "es",
    swedish: "sv", turkish: "tr", vietnamese: "vi"
  };
  return mapping[requested_language.toLowerCase()] || "";
}

// Main keyword extraction logic
function extract(
  str,
  options = {
    remove_digits: true,
  ...
  for (let x = 0; x < words.length; x++) {
    let w = words[x].match(/https?:\/\/.*[\r\n]*/g)
      ? words[x]
      : words[x].replace(/\.|,|;|!|\?|\(|\)|:|"|^'|'$|“|”|‘|’/g, "");
    if (w.length === 1) w = w.replace(/_|@|&|#/g, "");
    const digits_match = w.match(/\d/g);
    if (remove_digits && digits_match && digits_match.length === w.length) w = "";
    if (w.length > 0) {
...
// Expose to global scope for offline app
